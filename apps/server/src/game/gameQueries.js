...
export const baseCells = seat => baseCellsForSeat(seat, GAME_CONFIG.boardSize);

export function inBase(x, z) {
  return [1, 2].some(seat => baseCells(seat).some(cell => cell.x === x && cell.z === z));
}
...
export function deploymentCell(seat, x, z) {
  return isDeploymentCell(seat, x, z, GAME_CONFIG.boardSize);
}
